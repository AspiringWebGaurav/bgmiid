...
- **Dark & Light Themes**: Beautiful UI with multiple themes.
...
- **Deployment & Hosting**: Node.js ready
...
3. **Environment Setup**
   Copy `.env.example` to `.env` in the root directory and add your credentials:
   ```bash
   cp .env.example .env
   ```
...
Deploy the `web/build` folder to your favorite hosting provider (Vercel, Firebase Hosting, Netlify, etc.) or run the server with `npm start`.
